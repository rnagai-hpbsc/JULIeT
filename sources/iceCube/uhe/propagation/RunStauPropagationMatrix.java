...
public class RunStauPropagationMatrix {
    ...
    /** 
      ...
      */
    public RunStauPropagationMatrix(double nadirAngle,
            int intSwitch,int decaySwitch, int mediumNumber, int mass, double lifetime) throws IOException {
        ...
        if((0<intSwitch && intSwitch <512) && (0<decaySwitch && decaySwitch <512)){
            ...
            stau = new DynamicParticle(2,1); // stau ** added

            System.out.println("Initial value of the stau; mass: " + stau.getMass() + " lifetime: " + stau.getLifeTime() );

            stau.setMass(mass);

            if (lifetime < 0) {
                lifetime = Double.POSITIVE_INFINITY; // lifetime. highding the same variable in Particle class
            }
            stau.setLifeTime(lifetime);
            System.out.println("Check lifetime:" + stau.getLifeTime());

            // Generate the Propagation Matrix
            propMtx = new PropagationMatrixStau(nuE,nuMu,nuTau,e,mu,tau,pi,
                    s,intSwitch,decaySwitch,stau);
        }
        }

    /** Constructor to do nothing - for subclass modules */
    public RunStauPropagationMatrix(){};

    public RunStauPropagationMatrix(double nadirAngle,
            int intSwitch,int decaySwitch, int mediumNumber, int mass, double lifetime,
        ...
        if((0<intSwitch && intSwitch <512) && (0<decaySwitch && decaySwitch <512)){
            ...
            stau.setLifeTime(lifetime);
            System.out.println(stau.getLifeTime());

            // Generate the Propagation Matrix
            System.err.println("Setting the neutrino factor " + neutrinoFactor);
            propMtx = new PropagationMatrixStau(nuE,nuMu,nuTau,e,mu,tau,pi,
                    s,intSwitch,decaySwitch,neutrinoFactor,stau);
            //propMtx.setStauMass(mass);
        }
        }
    ...
    public void traceParticles(double trajectoryLength){
        ...
        for(section=1;section<=5;section++){
            ...
            for(int i=0;i<10;i++){
                ...
                System.out.println("After propMtx propagateDX.");
                l += deltaL;
                s.setParticleLocation(l);

                System.err.println("Location " + l/100.0 + " [m] "  + (trajectoryLength-l)/100.0 + " [m] to go.");
                //System.err.println("nuMu to nuMu " + propMtx.getFnuMuToNuMu(300,300) + " nuMu to nuTau " + propMtx.getFnuMuToNuTau(300,300));
                System.err.println("stau to stau " + propMtx.getFstauToStau(300,300) + " stau to Tau " + propMtx.getFstauToTau(300,300));
                //System.err.println("stau to nue " + propMtx.getFstauToNuE(300,300) + " stau to nuTau " + propMtx.getFstauToNuTau(300,300));
                //System.err.println("stau to stau " + propMtx.getFstauToStau(500,100) + " stau to Tau " + propMtx.getFstauToTau(300,300));
                if (propMtx.getFstauToStau(100,100) > 1.0) 
                    System.err.println("Warning!! Probability is greater than 1! ");
            }
            ...
            for(int i=0;i<k;i++){
                ...
                System.err.println("Location " + l/100.0 + " [m] "  + (trajectoryLength-l)/100.0 + " [m] to go.");
                //System.err.println("nuMu to nuMu " + propMtx.getFnuMuToNuMu(300,300) + " nuMu to nuTau " + propMtx.getFnuMuToNuTau(300,300));
                System.err.println("stau to stau " + propMtx.getFstauToStau(100,100) + " stau to Tau " + propMtx.getFstauToTau(300,300));
                //System.err.println("stau to nue " + propMtx.getFstauToNuE(300,300) + " stau to nuTau " + propMtx.getFstauToNuTau(300,300));
                System.err.println("stau to stau " + propMtx.getFstauToStau(500,100) + " stau to Tau " + propMtx.getFstauToTau(300,300));
                if (propMtx.getFstauToStau(100,100) > 1.0) 
                    System.err.println("Warning!! Probability is greater than 1! ");
    ...
    public void saveMatrix(DataOutputStream out) throws IOException {
        ...
        for(iLogE=0;iLogE<dimension;iLogE++){
            ...
            for(jLogE=0;jLogE<=iLogE;jLogE++){
                ...
                /* stau */
                out.writeDouble(propMtx.getFstauToNuE(iLogE,jLogE));
                out.writeDouble(propMtx.getFstauToNuMu(iLogE,jLogE));
                out.writeDouble(propMtx.getFstauToNuTau(iLogE,jLogE));
                out.writeDouble(propMtx.getFstauToE(iLogE,jLogE));
                out.writeDouble(propMtx.getFstauToMu(iLogE,jLogE));
                out.writeDouble(propMtx.getFstauToTau(iLogE,jLogE));
                out.writeDouble(propMtx.getFstauToHadron(iLogE,jLogE));
                out.writeDouble(propMtx.getFstauToStau(iLogE,jLogE));
                //System.out.println(iLogE + ", " + jLogE + ": " + propMtx.getFstauToStau(iLogE,jLogE));
            }
        }
