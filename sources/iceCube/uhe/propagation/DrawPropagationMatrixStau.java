...
public class DrawPropagationMatrixStau {
    ...
    // for stau 
    double[][] FstauToNuE, FstauToNuMu, FstauToNuTau, FstauToE, FstauToMu, FstauToTau, FstauToHadron, FstauToStau;
    private final static int STAU = 128;

    /** Constructor */
    public DrawPropagationMatrixStau(int inputParticle, int outputParticle,
    ...
    public void readMatrix(DataInputStream in) throws IOException {
        ...
        for(iLogE=0;iLogE<dimension;iLogE++){
            ...
            for(jLogE=0;jLogE<=iLogE;jLogE++){
                ...
                /** For Staus -begin **/
                FstauToNuE[iLogE][jLogE] = in.readDouble( );
                FstauToNuMu[iLogE][jLogE] = in.readDouble( );
                FstauToNuTau[iLogE][jLogE] = in.readDouble( );
                FstauToE[iLogE][jLogE] = in.readDouble( );
                FstauToMu[iLogE][jLogE] = in.readDouble( );
                FstauToTau[iLogE][jLogE] = in.readDouble( );
                FstauToHadron[iLogE][jLogE] = in.readDouble( );
                FstauToStau[iLogE][jLogE] = in.readDouble( );
                /** For Staus -end **/
            }
        }
    ...
    public void drawOnXfig( ){
        ...
        System.out.println("inLogE " + inLogE + " : " + (Particle.getLogEnergyMinimum() + Particle.getDeltaLogEnergy()*(double)inLogE)); 
        ...
        /* For Stau : begin */
        if (((inputParticle & STAU) == STAU) && ((outputParticle & NU_E) == NU_E)) {
            System.out.println("lnpt x");
            System.out.println("lnth x");
            System.out.println("lncl x");
            for (jLogE=0;jLogE<dimension;jLogE++) {
                count = 0.0;
                if (inLogE==-1){
                    for (iLogE=jLogE;iLogE<dimension;iLogE++) {
                        if (FstauToNuE[iLogE][jLogE]>0.0) { 
                            count += FstauToNuE[iLogE][jLogE];
                            sumF[jLogE] += FstauToNuE[iLogE][jLogE];
                        }
                    }
                } else {
                    count += FstauToNuE[inLogE][jLogE];
                    sumF[jLogE] += FstauToNuE[inLogE][jLogE];
                }
                if (count<=0.0) logCount = logYmin;
                else logCount = Math.log(count)/Math.log(10.0);

                logE = Particle.getLogEnergyMinimum() + Particle.getDeltaLogEnergy()*(double)jLogE;
        ...
        if (((inputParticle & STAU) == STAU) && ((outputParticle & NU_MU) == NU_MU)) {
            System.out.println("lnpt x");
            System.out.println("lnth x");
            System.out.println("lncl x");
            for (jLogE=0;jLogE<dimension;jLogE++) {
                ...
                if (inLogE==-1) {
                    for (iLogE=jLogE; iLogE<dimension; iLogE++) { 
                        if (FstauToNuMu[iLogE][jLogE]>0.0) { 
                            count += FstauToNuMu[iLogE][jLogE]; 
                            sumF[jLogE] += FstauToNuMu[iLogE][jLogE];
                        }
                    }
                } else {
                    count += FstauToNuMu[inLogE][jLogE];
                    sumF[jLogE] += FstauToNuMu[inLogE][jLogE]; 
                }
                if (count<=0.0) logCount = logYmin;
                else logCount = Math.log(count)/Math.log(10.0);
        ...
        if (((inputParticle & STAU) == STAU) && ((outputParticle & NU_TAU) == NU_TAU)) { 
            System.out.println("lnpt x");
            System.out.println("lnth x");
            System.out.println("lncl x");
            for (jLogE=0;jLogE<dimension;jLogE++) { 
                ...
                if (inLogE==-1) { 
                    for (iLogE=jLogE; iLogE<dimension; iLogE++) { 
                        if (FstauToNuTau[iLogE][jLogE]>0.0) { 
                            count += FstauToNuTau[iLogE][jLogE];
                            sumF[jLogE] += FstauToNuTau[iLogE][jLogE];
                        }
                    }
                } else {
                    count += FstauToNuTau[inLogE][jLogE];
                    sumF[jLogE] += FstauToNuTau[inLogE][jLogE];
                }
                if (count<=0.0) logCount = logYmin;
                else logCount = Math.log(count)/Math.log(10.0);
        ...
        if (((inputParticle & STAU) == STAU) && ((outputParticle & EL) == EL)) { 
            System.out.println("lnpt x");
            System.out.println("lnth x");
            System.out.println("lncl x");
            for (jLogE=0; jLogE<dimension; jLogE++) { 
                ...
                if (inLogE==-1) { 
                    for (iLogE=jLogE; iLogE<dimension; iLogE++) {
                        if (FstauToE[iLogE][jLogE]>0.0) {
                            count += FstauToE[iLogE][jLogE];
                            sumF[jLogE] += FstauToE[iLogE][jLogE];
                        }
                    }
                } else { 
                    count += FstauToE[inLogE][jLogE];
                    sumF[jLogE] += FstauToE[inLogE][jLogE];
                }
                if (count<=0.0) logCount = logYmin;
                else logCount = Math.log(count)/Math.log(10.0);
        ...
        if (((inputParticle & STAU) == STAU) && ((outputParticle & MU) == MU)) { 
            System.out.println("lnpt x");
            System.out.println("lnth x");
            System.out.println("lncl x");
            for (jLogE=0; jLogE<dimension; jLogE++) { 
                ...
                if (inLogE==-1) {
                    for (iLogE=jLogE; iLogE<dimension; iLogE++) {
                        if (FstauToMu[iLogE][jLogE]>0.0) { 
                            count += FstauToMu[iLogE][jLogE];
                            sumF[jLogE] += FstauToMu[iLogE][jLogE];
                        }
                    }
                } else { 
                    count += FstauToMu[inLogE][jLogE];
                    sumF[jLogE] += FstauToMu[inLogE][jLogE];
                }
                if (count<=0.0) logCount = logYmin;
                else logCount = Math.log(count)/Math.log(10.0);
        ...
        if (((inputParticle & STAU) == STAU) && ((outputParticle & TAU) == TAU)) { 
            System.out.println("lnpt x");
            System.out.println("lnth x");
            System.out.println("lncl x");
            for (jLogE=0; jLogE<dimension; jLogE++) { 
                ...
                if (inLogE==-1) { 
                    for (iLogE=jLogE; iLogE<dimension; iLogE++) { 
                        if (FstauToTau[iLogE][jLogE]>0.0) { 
                            count += FstauToTau[iLogE][jLogE];
                            sumF[jLogE] += FstauToTau[iLogE][jLogE];
                        }
                    }
                } else { 
                    count += FstauToTau[inLogE][jLogE];
                    sumF[jLogE] += FstauToTau[inLogE][jLogE];
                }
                if (count<=0.0) logCount = logYmin;
                else logCount = Math.log(count)/Math.log(10.0);
        ...
        if (((inputParticle & STAU) == STAU) && ((outputParticle & HADRON) == HADRON)) { 
            System.out.println("lnpt x");
            System.out.println("lnth x");
            System.out.println("lncl x");
            for (jLogE=0; jLogE<dimension; jLogE++) { 
                ...
                if (inLogE==-1) { 
                    for (iLogE=jLogE; iLogE<dimension; iLogE++) { 
                        if (FstauToHadron[iLogE][jLogE]>0.0) { 
                            count += FstauToHadron[iLogE][jLogE];
                            sumF[jLogE] += FstauToHadron[iLogE][jLogE];
                        }
                    }
                } else { 
                    count += FstauToHadron[inLogE][jLogE];
                    sumF[jLogE] += FstauToHadron[inLogE][jLogE]; 
                }
                if (count<=0.0) logCount = logYmin;
                else logCount = Math.log(count)/Math.log(10.0);
        ...
        if (((inputParticle & STAU) == STAU) && ((outputParticle & STAU) == STAU)) {
            System.out.println("lnpt x");
            System.out.println("lnth x");
            System.out.println("lncl x");
            for (jLogE=0; jLogE<dimension; jLogE++) { 
                ...
                if (inLogE==-1) { 
                    for (iLogE=jLogE; iLogE<dimension; iLogE++) {
                        ...
                        if (FstauToStau[iLogE][jLogE]>0.0) {
                            count += FstauToStau[iLogE][jLogE];
                            sumF[jLogE] += FstauToStau[iLogE][jLogE];
                        }
                    }
                } else { 
                    count += FstauToStau[inLogE][jLogE];
                    sumF[jLogE] += FstauToStau[inLogE][jLogE];
                    //System.out.println(FstauToStau[inLogE][jLogE]);
                }
                if (count<=0.0) logCount = logYmin;
                else logCount = Math.log(count)/Math.log(10.0);
    ...
    {   
        ...
        double logYmin = -12.0;
        double logYmax = 1.0;
        DrawPropagationMatrixStau draw;
        ...
        if (args.length==3) {
            ...
            System.out.println("Usage: DrawPropagationMatrixStau file-name inputP outputP logYmin logYmax (inputenergy)");
            System.exit(0);
        } else {
            fileName = args[0];
            inP = Integer.valueOf(args[1]).intValue();
        ...
        if(args.length == 6){
            ...
            draw = new DrawPropagationMatrixStau(inP,outP,logYmin,logYmax,inLogE);
        }else{
            draw = new DrawPropagationMatrixStau(inP,outP,logYmin,logYmax);
        }
