...
*** Added ***
stauToNuE     : prohibited 
stauToNuMu    : prohibited
stauToNuTau   : prohibited
stauToE       : by Pair Creation and Bremsstrahlung
stauToMu      : by Pair Creation
stauToTau     : by Pair Creation                       Decay
stauToHadron  : by PhotoNuclear Interactions 
stauToStau    : by survival
...
public class PropagationMatrixStau {
    ...
    // added for stau 
    protected int staumass = 100;
    protected double staulifetime = Double.POSITIVE_INFINITY; 
    // infinitsimal propagation:  
    ...
    protected double[][] stauToStau;
    // finite propagation: 
    protected double[][] FstauToNuE;
    protected double[][] FstauToNuMu;
    protected double[][] FstauToNuTau;
    protected double[][] FstauToE;
    protected double[][] FstauToMu;
    protected double[][] FstauToTau;
    protected double[][] FstauToHadron;
    protected double[][] FstauToStau;
    // storing calculated matrices: 
    protected double[][] SstauToNuE;
    protected double[][] SstauToNuMu;
    protected double[][] SstauToNuTau;
    protected double[][] SstauToE;
    protected double[][] SstauToMu;
    protected double[][] SstauToTau;
    protected double[][] SstauToHadron;
    protected double[][] SstauToStau;
    ...
    // added for stau 
    protected final static int STAUDECAY_FLAG = 4;
    ...
    // added for stau
    private InteractionsMatrix stauToEPairCMtx = null;
    private String stauToEPairCMtxObjectFile = "stauToePairCreationMtx" + String.valueOf(staumass) + "GeV";
    private InteractionsMatrix stauToMuPairCMtx = null;
    private String stauToMuPairCMtxObjectFile = "stauTomuPairCreationMtx" + String.valueOf(staumass) + "GeV";
    private InteractionsMatrix stauToTauPairCMtx = null;
    private String stauToTauPairCMtxObjectFile = "stauTotauPairCreationMtx" + String.valueOf(staumass) + "GeV";
    ...
    // added for stau
    private InteractionsMatrix stauBremssMtx = null;
    private String stauBremssMtxObjectFile = "stauBremsstrahlungMtx" + String.valueOf(staumass) + "GeV";
    ...
    private InteractionsMatrix stauKnockOnMtx = null;
    private String stauKnockOnMtxObjectFile = "stauKnockOnElectronsMtx" + String.valueOf(staumass) + "GeV";
    ...
    // added for stau
    private InteractionsMatrix stauPhotoNuclMtx = null;
    private String stauPhotoNuclMtxObjectFile = "stauPhotoNuclearMtx" + String.valueOf(staumass) + "GeV";
    ...
    // added for stau
    private StauDecayMatrix stauDecayMtx = null;
    ...
    DynamicParticle stau = null; // added ** stau
    ...
    public PropagationMatrixStau(Particle nuE, Particle nuMu, Particle nuTau,
            ...
            String nuCCMtxObjectFile, String nuNCMtxObjectFile, DynamicParticle stau) 
        throws IOException{

        this.stau = stau;
        this.staumass = (int)stau.getMass(); 
        this.stauToEPairCMtxObjectFile = "stauToePairCreationMtx" + String.valueOf(staumass) + "GeV";
        this.stauToMuPairCMtxObjectFile = "stauTomuPairCreationMtx" + String.valueOf(staumass) + "GeV";
        this.stauToTauPairCMtxObjectFile = "stauTotauPairCreationMtx" + String.valueOf(staumass) + "GeV";
        this.stauBremssMtxObjectFile = "stauBremsstrahlungMtx" + String.valueOf(staumass) + "GeV";
        this.stauPhotoNuclMtxObjectFile = "stauPhotoNuclearMtx" + String.valueOf(staumass) + "GeV";
        this.stauKnockOnMtxObjectFile = "stauKnockOnMtx" + String.valueOf(staumass) + "GeV";
        System.err.println("Stau Mass Set: " + this.staumass); 
        System.err.println("Stau filename: " + this.stauToEPairCMtxObjectFile); 
        ...
        if((interactionsSwitch & PAIRC_FLAG) == PAIRC_FLAG){
            ...
            in.close( );
            // added for stau
            objectFile = pathName[s.getMaterialNumber()].concat(stauToEPairCMtxObjectFile);
            System.err.println("Set object file name." + objectFile);
            in = ClassLoader.getSystemResourceAsStream(objectFile);
            stauToEPairCMtx = InteractionsMatrixInput.inputInteractionsMatrix(in);
            in.close();
        }else{
        ...
        if((interactionsSwitch & PAIRCH_FLAG) == PAIRCH_FLAG){
            ...
            in.close( );
            // added for stau
            objectFile = pathName[s.getMaterialNumber()].concat(stauToMuPairCMtxObjectFile);
            System.err.println("Set object file name." + objectFile);
            in = ClassLoader.getSystemResourceAsStream(objectFile);
            stauToMuPairCMtx = InteractionsMatrixInput.inputInteractionsMatrix(in);
            in.close();
            objectFile = pathName[s.getMaterialNumber()].concat(stauToTauPairCMtxObjectFile);
            System.err.println("Set object file name." + objectFile);
            in = ClassLoader.getSystemResourceAsStream(objectFile);
            stauToTauPairCMtx = InteractionsMatrixInput.inputInteractionsMatrix(in);
            in.close();
        }else{
        ...
        if((interactionsSwitch & BREMSS_FLAG) == BREMSS_FLAG){
            ...
            in.close( );
            // added for stau
            objectFile = pathName[s.getMaterialNumber()].concat(stauBremssMtxObjectFile);
            System.err.println("Set object file name." + objectFile);
            in = ClassLoader.getSystemResourceAsStream(objectFile);
            stauBremssMtx = InteractionsMatrixInput.inputInteractionsMatrix(in);
            in.close( );
        }else{
        ...
        if((interactionsSwitch & KNOCK_FLAG) == KNOCK_FLAG){
            ...
            in.close( );
            //objectFile = pathName[s.getMaterialNumber()].concat(stauKnockOnMtxObjectFile);
            //System.err.println("Set object file name." + objectFile);
            //in = ClassLoader.getSystemResourceAsStream(objectFile);
            //stauKnockOnMtx = InteractionsMatrixInput.inputInteractionsMatrix(in);
            //in.close();
        }else{
        ...
        if((interactionsSwitch & PHOTO_FLAG) == PHOTO_FLAG){
            ...
            in.close( );
            // added for stau
            objectFile = pathName[s.getMaterialNumber()].concat(stauPhotoNuclMtxObjectFile);
            System.err.println("Set object file name." + objectFile);
            in = ClassLoader.getSystemResourceAsStream(objectFile);
            stauPhotoNuclMtx = InteractionsMatrixInput.inputInteractionsMatrix(in);
            in.close( );
        }else{
        ...
        // added Stau-Decay 
        if((decaySwitch & STAUDECAY_FLAG) == STAUDECAY_FLAG){
            stauDecayMtx = new StauDecayMatrix(stau);
            System.out.println(stau.getMass()+"GeV");
            System.out.println(stau.getLifeTime()+"s");
            int iLogE;
            for (iLogE=0;iLogE<stau.getDimensionOfLogEnergyMatrix();iLogE++){
                // LifeTime [sec] 
                stauDecayMtx.setLifeTimeMatrix(iLogE);
                // Decay Matrix 
                int jLogE; 
                for (jLogE=0;jLogE<stau.getDimensionOfLogEnergyMatrix();jLogE++){
                    stauDecayMtx.setStauDecayMatrix(iLogE,jLogE);
                }
            }
        } else {
            System.err.println("Stau decay switched off.");
        }
        ...
        intProbStau = new double[dimension];
        /** For Glashow Resonance **/
        ...
        temp_stau = new double[10][dimension][dimension];
        ...
        stauToTau = new double[dimension][dimension];
        stauToHadron = new double[dimension][dimension];
        stauToStau = new double[dimension][dimension];
        FstauToNuE = new double[dimension][dimension];
        FstauToNuMu = new double[dimension][dimension];
        FstauToNuTau = new double[dimension][dimension];
        FstauToE = new double[dimension][dimension];
        FstauToMu = new double[dimension][dimension];
        FstauToTau = new double[dimension][dimension];
        FstauToHadron = new double[dimension][dimension];
        FstauToStau = new double[dimension][dimension];
        SstauToNuE = new double[dimension][dimension];
        SstauToNuMu = new double[dimension][dimension];
        SstauToNuTau = new double[dimension][dimension];
        SstauToE = new double[dimension][dimension];
        SstauToMu = new double[dimension][dimension];
        SstauToTau = new double[dimension][dimension];
        SstauToHadron = new double[dimension][dimension];
        SstauToStau = new double[dimension][dimension];
    ...
    public PropagationMatrixStau(Particle nuE, Particle nuMu, Particle nuTau,
            ...
            Particle pi,  ParticlePoint s,
            int interactionsSwitch, int decaySwitch,
            double neutrinoFactor, DynamicParticle stau)
        throws IOException{
        this(nuE, nuMu, nuTau, e, mu, tau, pi, s,interactionsSwitch, decaySwitch, neutrinoFactor,
                nuCCMtxObjectCCH5File,nuNCMtxObjectCCH5File, stau);
    }
    ...
    public PropagationMatrixStau(Particle nuE, Particle nuMu, Particle nuTau,
            ...
            Particle pi,  ParticlePoint s,
            int interactionsSwitch, int decaySwitch,
            String nuCCMtxObjectFile, String nuNCMtxObjectFile, DynamicParticle stau) throws IOException{
        this(nuE, nuMu, nuTau, e, mu, tau, pi, s,interactionsSwitch, decaySwitch, 1.0,
                nuCCMtxObjectFile,nuNCMtxObjectFile, stau);
    }
    ...
    public PropagationMatrixStau(Particle nuE, Particle nuMu, Particle nuTau,
            Particle e,   Particle mu,   Particle tau,
            Particle pi,  ParticlePoint s,
            int interactionsSwitch, int decaySwitch, DynamicParticle stau) throws IOException{
        this(nuE, nuMu, nuTau, e, mu, tau, pi, s,interactionsSwitch, decaySwitch, 1.0,
                nuCCMtxObjectCCH5File,nuNCMtxObjectCCH5File, stau);
    }
    ...
    public void init( ){
        ...
        for(iLogE=0;iLogE<dimension;iLogE++){
            ...
            for(jLogE=0;jLogE<dimension;jLogE++){
                ...
                // added for stau
                FstauToNuE[iLogE][jLogE] = 0.0;
                FstauToNuMu[iLogE][jLogE] = 0.0;
                FstauToNuTau[iLogE][jLogE] = 0.0;
                FstauToE[iLogE][jLogE] = 0.0;
                FstauToMu[iLogE][jLogE] = 0.0;
                FstauToTau[iLogE][jLogE] = 0.0;
                FstauToHadron[iLogE][jLogE] = 0.0;
                FstauToStau[iLogE][jLogE] = 0.0;
    ...
    public void initALL( ){
        ...
        for(iLogE=0;iLogE<dimension;iLogE++){
            ...
            for(jLogE=0;jLogE<dimension;jLogE++){
                ...
                // added for stau
                SstauToNuE[iLogE][jLogE] = 0.0;
                SstauToNuMu[iLogE][jLogE] = 0.0;
                SstauToNuTau[iLogE][jLogE] = 0.0;
                SstauToE[iLogE][jLogE] = 0.0;
                SstauToMu[iLogE][jLogE] = 0.0;
                SstauToTau[iLogE][jLogE] = 0.0;
                SstauToHadron[iLogE][jLogE] = 0.0;
                SstauToStau[iLogE][jLogE] = 0.0;
    ...
    public void calculateTransferMatrix( ){
        ...
        for(kLogE=0;kLogE<dimension;kLogE++){
            ...
            if((interactionsSwitch & PAIRC_FLAG) == PAIRC_FLAG){
                ...
                intProbStau[kLogE] += dX*s.NA/massNumber*stauToEPairCMtx.getSigmaMatrix(kLogE); // added
            }
            if((interactionsSwitch & PAIRCH_FLAG) == PAIRCH_FLAG){
                intProbMu[kLogE] += 
                    dX*s.NA/massNumber*(
                ...
                intProbStau[kLogE] += 
                    dX*s.NA/massNumber*(
                            stauToMuPairCMtx.getSigmaMatrix(kLogE)+
                            stauToTauPairCMtx.getSigmaMatrix(kLogE)); // added 
            }
            if((interactionsSwitch & BREMSS_FLAG) == BREMSS_FLAG){
                ...
                intProbStau[kLogE] += dX*s.NA/massNumber*stauBremssMtx.getSigmaMatrix(kLogE); // added 
            }
            if((interactionsSwitch & KNOCK_FLAG) == KNOCK_FLAG){
                ...
                //intProbStau[kLogE] += dX*s.NA/massNumber*stauKnockOnMtx.getSigmaMatrix(kLogE); // added
            }
            if((interactionsSwitch & PHOTO_FLAG) == PHOTO_FLAG){
                ...
                intProbStau[kLogE] += dX*s.NA/massNumber*stauPhotoNuclMtx.getSigmaMatrix(kLogE); // added 
            }
            /** For Glashow Resonance -begin **/
            if((interactionsSwitch & GR_FLAG) == GR_FLAG){
            ...
            if((decaySwitch & STAUDECAY_FLAG) == STAUDECAY_FLAG) { 
                intProbStau[kLogE] += 
                    dX/(c*stauDecayMtx.getLifeTimeMatrix(kLogE)*s.getMediumDensity( ));
            }
            
            //System.out.println("intProbStau " + intProbStau[kLogE]);
            ...
            for(jLogE=0;jLogE<=kLogE;jLogE++){
                ...
                stauToE[kLogE][jLogE] = 0.0; // added for stau
                if((interactionsSwitch & BREMSS_FLAG) == BREMSS_FLAG){
                    muToE[kLogE][jLogE] += 
                    ...
                    stauToE[kLogE][jLogE] += 
                        dX*s.NA/massNumber*stauBremssMtx.getTransferMatrix(kLogE, jLogE); // added 
                }
                ...
                if((interactionsSwitch & PAIRC_FLAG) == PAIRC_FLAG){
                    ...
                    stauToE[kLogE][jLogE] += 2.0*
                        dX*s.NA/massNumber*stauToEPairCMtx.getTransferMatrix(kLogE,jLogE); // added 
                }
                ...
                if((interactionsSwitch & PAIRCH_FLAG) == PAIRCH_FLAG){
                    ...
                    stauToMu[kLogE][jLogE] += 2.0*
                        dX*s.NA/massNumber*stauToMuPairCMtx.getTransferMatrix(kLogE,jLogE); // added 
                }
                ...
                stauToTau[kLogE][jLogE] = 0.0; // added 
                if((interactionsSwitch & PAIRCH_FLAG) == PAIRCH_FLAG){
                    ...
                    stauToTau[kLogE][jLogE] += 2.0*
                        dX*s.NA/massNumber*stauToTauPairCMtx.getTransferMatrix(kLogE,jLogE); // added 
                }
                ...
                // stau decay 
                if((decaySwitch & STAUDECAY_FLAG) == STAUDECAY_FLAG){
                    stauToTau[kLogE][jLogE] += 
                        dX*stauDecayMtx.getStauToTauDecayMatrix(kLogE,jLogE)
                        /(c*stauDecayMtx.getLifeTimeMatrix(kLogE)*s.getMediumDensity( ));
                }

                // To Stau
                stauToStau[kLogE][jLogE] = 0.0;

                if((interactionsSwitch & PAIRCH_FLAG) == PAIRCH_FLAG){
                    stauToStau[kLogE][jLogE] += dX*s.NA/massNumber*(
                        stauToMuPairCMtx.getLeptonTransferMatrix(kLogE,jLogE)+
                        stauToTauPairCMtx.getLeptonTransferMatrix(kLogE,jLogE));
                }
                
                if((interactionsSwitch & PAIRC_FLAG) == PAIRC_FLAG){
                    stauToStau[kLogE][jLogE] += dX*s.NA/massNumber*
                        stauToEPairCMtx.getLeptonTransferMatrix(kLogE,jLogE);
                }

                if((interactionsSwitch & BREMSS_FLAG) == BREMSS_FLAG){
                    stauToStau[kLogE][jLogE] += dX*s.NA/massNumber*
                        stauBremssMtx.getLeptonTransferMatrix(kLogE,jLogE);
                }

                if((interactionsSwitch & PHOTO_FLAG) == PHOTO_FLAG){
                    stauToStau[kLogE][jLogE] += dX*s.NA/massNumber*
                        stauPhotoNuclMtx.getLeptonTransferMatrix(kLogE,jLogE);
                }
                ...
                stauToHadron[kLogE][jLogE] = 0.0; // added 
                if((interactionsSwitch & PHOTO_FLAG) == PHOTO_FLAG){
                    ...
                    stauToHadron[kLogE][jLogE] += dX*s.NA/massNumber*
                        stauPhotoNuclMtx.getTransferMatrix(kLogE,jLogE); // added 
                }
                if((decaySwitch & TAUDECAY_FLAG) == TAUDECAY_FLAG){
    ...
    public void propagateDX( ){
        ...
        for(iLogE=0;iLogE<dimension;iLogE++){
            ...
            for(jLogE=0;jLogE<=iLogE;jLogE++){
                ...
                dNFromStau = 0.0; // From Staus, added 
                ...
                for(kLogE=iLogE;kLogE>=jLogE;kLogE--){
                    ...
                    //stau to nuE // added
                    dNFromStau +=
                        FstauToNuE[iLogE][kLogE]*nuEToNuE[kLogE][jLogE]+
                        FstauToMu[iLogE][kLogE]*muToNuE[kLogE][jLogE] + 
                        FstauToTau[iLogE][kLogE]*tauToNuE[kLogE][jLogE];

                    debugStau = dNFromStau;
                ...
                dNFromStau += FstauToNuE[iLogE][jLogE]*(1.0-intProbNeutrino[jLogE]-intProbNuE[jLogE]);
                FstauToNuE[iLogE][jLogE] = dNFromStau;

                // Debug
                if (FstauToNuE[iLogE][jLogE] > 1.0) {
                    System.out.println("FstauToNuE["+iLogE+"]["+jLogE+"]: " + FstauToNuE[iLogE][jLogE]);
                    System.out.println("dNFromStau     : " + dNFromStau);
                    System.out.println("dNFromStau(N-1): " + debugStau);
                    //System.exit(0);
                }
                ...
                for(kLogE=iLogE;kLogE>=jLogE;kLogE--){
                    ...
                    //stau to nuMu
                    dNFromStau += 
                        FstauToNuE[iLogE][kLogE]*nuEToNuMu[kLogE][jLogE]+
                        FstauToNuMu[iLogE][kLogE]*nuMuToNuMu[kLogE][jLogE]+
                        FstauToMu[iLogE][kLogE]*muToNuMu[kLogE][jLogE] +
                        FstauToTau[iLogE][kLogE]*tauToNuMu[kLogE][jLogE];
                ...
                dNFromStau += FstauToNuMu[iLogE][jLogE]*(1.0-intProbNeutrino[jLogE]);
                FstauToNuMu[iLogE][jLogE] = dNFromStau;
                ...
                for(kLogE=iLogE;kLogE>=jLogE;kLogE--){
                    ...
                    //Stau to nuTau
                    dNFromStau += 
                        FstauToNuE[iLogE][kLogE]*nuEToNuTau[kLogE][jLogE]+
                        FstauToNuTau[iLogE][kLogE]*nuTauToNuTau[kLogE][jLogE] + 
                        FstauToTau[iLogE][kLogE]*tauToNuTau[kLogE][jLogE];
                ...
                dNFromStau += FstauToNuTau[iLogE][jLogE]*(1.0-intProbNeutrino[jLogE]);
                FstauToNuTau[iLogE][jLogE] = dNFromStau;
                ...
                for(kLogE=iLogE;kLogE>=jLogE;kLogE--){
                    ...
                    //Stau to Electrons
                    dNFromStau += 
                        FstauToNuE[iLogE][kLogE]*nuEToE[kLogE][jLogE]+
                        FstauToMu[iLogE][kLogE]*muToE[kLogE][jLogE]+ 
                        FstauToTau[iLogE][kLogE]*tauToE[kLogE][jLogE]+
                        FstauToStau[iLogE][kLogE]*stauToE[kLogE][jLogE];
                ...
                dNFromStau += FstauToE[iLogE][jLogE];
                FstauToE[iLogE][jLogE] = dNFromStau;
                ...
                for(kLogE=iLogE;kLogE>=jLogE;kLogE--){
                    ...
                    //Stau to Muons
                    dNFromStau += 
                        FstauToNuE[iLogE][kLogE]*nuEToMu[kLogE][jLogE]+
                        FstauToNuMu[iLogE][kLogE]*nuMuToMu[kLogE][jLogE]+
                        FstauToMu[iLogE][kLogE]*muToMu[kLogE][jLogE]+ 
                        FstauToTau[iLogE][kLogE]*tauToMu[kLogE][jLogE]+
                        FstauToStau[iLogE][kLogE]*stauToMu[kLogE][jLogE];
                ...
                dNFromStau += FstauToMu[iLogE][jLogE]*(1.0-intProbMu[jLogE]);
                FstauToMu[iLogE][jLogE] = dNFromStau;
                ...
                for(kLogE=iLogE;kLogE>=jLogE;kLogE--){
                    ...
                    //Stau to Tauons
                    dNFromStau += 
                        FstauToNuE[iLogE][kLogE]*nuEToTau[kLogE][jLogE] +
                        FstauToNuTau[iLogE][kLogE]*nuTauToTau[kLogE][jLogE] +
                        FstauToMu[iLogE][kLogE]*muToTau[kLogE][jLogE]+ 
                        FstauToTau[iLogE][kLogE]*tauToTau[kLogE][jLogE]+
                        FstauToStau[iLogE][kLogE]*stauToTau[kLogE][jLogE];
                ...
                dNFromStau += FstauToTau[iLogE][jLogE]*(1.0-intProbTau[jLogE]);
                FstauToTau[iLogE][jLogE] = dNFromStau;
                ...
                for(kLogE=iLogE;kLogE>=jLogE;kLogE--){
                    ...
                    //Stau to Hadrons
                    dNFromStau +=
                        FstauToNuE[iLogE][kLogE]*nuEToHadron[kLogE][jLogE] +
                        FstauToNuMu[iLogE][kLogE]*nuMuToHadron[kLogE][jLogE] +
                        FstauToNuTau[iLogE][kLogE]*nuTauToHadron[kLogE][jLogE] +
                        FstauToMu[iLogE][kLogE]*muToHadron[kLogE][jLogE] + 
                        FstauToTau[iLogE][kLogE]*tauToHadron[kLogE][jLogE] +
                        FstauToStau[iLogE][kLogE]*stauToHadron[kLogE][jLogE];
                }
                ...
                dNFromStau += FstauToHadron[iLogE][jLogE]; 
                FstauToHadron[iLogE][jLogE] = dNFromStau;
                ...
                for(kLogE=iLogE;kLogE>=jLogE;kLogE--){
                    dNFromStau +=  FstauToStau[iLogE][kLogE]*stauToStau[kLogE][jLogE];
                }

                dNFromStau += FstauToStau[iLogE][jLogE]*(1.0-intProbStau[jLogE]);

                if (dNFromStau>1.0){
                //if (iLogE==54 && jLogE==54) {
                    //for (kLogE=iLogE;kLogE>=jLogE;kLogE--){
                    //    System.out.println("FstauToStau["+iLogE+"]["+kLogE+"]: " + FstauToStau[iLogE][kLogE]);
                    //    System.out.println(" stauToStau["+kLogE+"]["+jLogE+"]: " +  stauToStau[kLogE][jLogE]);
                    //}
                    System.out.println("FstauToStau["+iLogE+"]["+jLogE+"]: " + FstauToStau[iLogE][jLogE]);
                    System.out.println("FtauToTau["+iLogE+"]["+jLogE+"]  : " + FtauToTau[iLogE][jLogE]);
                    System.out.println("stauToStau["+iLogE+"]["+jLogE+"]: " + stauToStau[iLogE][jLogE]);
                    System.out.println("tauToTau["+iLogE+"]["+jLogE+"]  : " + tauToTau[iLogE][jLogE]);
                    System.out.println("intProbStau["+jLogE+"]: " + intProbStau[jLogE]);
                    System.out.println("intProbTau["+jLogE+"] : " + intProbTau[jLogE]);
                    System.out.println("FstauToStau["+iLogE+"]["+jLogE+"]*(1.0-intProbStau[jLogE]): " + FstauToStau[iLogE][jLogE]*(1.0-intProbStau[jLogE]));
                    System.out.println("FstauToStau: " + dNFromStau);
                    System.out.println("********************");
                    System.out.println("E   PairC : " + (stauToEPairCMtx.getSigmaMatrix(iLogE) - stauToEPairCMtx.getLeptonTransferMatrix(iLogE,iLogE)));
                    System.out.println("Mu  PairC : " + (stauToMuPairCMtx.getSigmaMatrix(iLogE) - stauToMuPairCMtx.getLeptonTransferMatrix(iLogE,iLogE)));
                    System.out.println("Tau PairC : " + (stauToTauPairCMtx.getSigmaMatrix(iLogE) - stauToTauPairCMtx.getLeptonTransferMatrix(iLogE,iLogE)));
                    System.out.println("Bremss    : " + (stauBremssMtx.getSigmaMatrix(iLogE) - stauBremssMtx.getLeptonTransferMatrix(iLogE,iLogE)));
                    System.out.println("PhotoNucl : " + (stauPhotoNuclMtx.getSigmaMatrix(iLogE) - stauPhotoNuclMtx.getLeptonTransferMatrix(iLogE,iLogE)));
                    System.out.println("********************");
                    //System.exit(0);
                }
                FstauToStau[iLogE][jLogE] = dNFromStau;
    ...
    public void propagateDXpowered( ){
        ...
        for(iLogE=0;iLogE<dimension;iLogE++){
            for(jLogE=0;jLogE<=iLogE;jLogE++){
                temp_stau[0][iLogE][jLogE] = 0.0;
                for(kLogE=iLogE;kLogE>=jLogE;kLogE--){
                    temp_stau[0][iLogE][jLogE] +=
                        FstauToNuE[iLogE][kLogE]*FnuEToNuE[kLogE][jLogE]+
                        FstauToNuMu[iLogE][kLogE]*FnuMuToNuE[kLogE][jLogE]+
                        FstauToNuTau[iLogE][kLogE]*FnuTauToNuE[kLogE][jLogE]+
                        FstauToMu[iLogE][kLogE]*FmuToNuE[kLogE][jLogE]+
                        FstauToTau[iLogE][kLogE]*FtauToNuE[kLogE][jLogE]+
                        FstauToStau[iLogE][kLogE]*FstauToNuE[kLogE][jLogE];
                }
            }
        }
        ...
        for(iLogE=0;iLogE<dimension;iLogE++){
            for(jLogE=0;jLogE<=iLogE;jLogE++){
                temp_stau[1][iLogE][jLogE] = 0.0;
                for(kLogE=iLogE;kLogE>=jLogE;kLogE--){
                    temp_stau[1][iLogE][jLogE] += 
                        FstauToNuE[iLogE][kLogE]*FnuEToNuMu[kLogE][jLogE]+
                        FstauToNuMu[iLogE][kLogE]*FnuMuToNuMu[kLogE][jLogE]+
                        FstauToNuTau[iLogE][kLogE]*FnuTauToNuMu[kLogE][jLogE]+
                        FstauToMu[iLogE][kLogE]*FmuToNuMu[kLogE][jLogE]+
                        FstauToTau[iLogE][kLogE]*FtauToNuMu[kLogE][jLogE]+
                        FstauToStau[iLogE][kLogE]*FstauToNuMu[kLogE][jLogE];
                }
            }
        }
        ...
        for(iLogE=0;iLogE<dimension;iLogE++){
            for(jLogE=0;jLogE<=iLogE;jLogE++){
                temp_stau[2][iLogE][jLogE] = 0.0;
                for(kLogE=iLogE;kLogE>=jLogE;kLogE--){
                    temp_stau[2][iLogE][jLogE] += 
                        /** For Glashow Resonance **/
                        FstauToNuE[iLogE][kLogE]*FnuEToNuTau[kLogE][jLogE]+
                        FstauToNuMu[iLogE][kLogE]*FnuMuToNuTau[kLogE][jLogE]+
                        FstauToNuTau[iLogE][kLogE]*FnuTauToNuTau[kLogE][jLogE]+
                        FstauToMu[iLogE][kLogE]*FmuToNuTau[kLogE][jLogE]+
                        FstauToTau[iLogE][kLogE]*FtauToNuTau[kLogE][jLogE]+
                        FstauToStau[iLogE][kLogE]*FstauToNuTau[kLogE][jLogE];
                }
            }
        }
        ...
        for(iLogE=0;iLogE<dimension;iLogE++){
            for(jLogE=0;jLogE<=iLogE;jLogE++){
                temp_stau[3][iLogE][jLogE] = 0.0;
                for(kLogE=iLogE;kLogE>=jLogE;kLogE--){
                    temp_stau[3][iLogE][jLogE] += 
                        FstauToNuE[iLogE][kLogE]*FnuEToE[kLogE][jLogE]+
                        FstauToNuMu[iLogE][kLogE]*FnuMuToE[kLogE][jLogE]+
                        FstauToNuTau[iLogE][kLogE]*FnuTauToE[kLogE][jLogE]+
                        FstauToMu[iLogE][kLogE]*FmuToE[kLogE][jLogE]+
                        FstauToTau[iLogE][kLogE]*FtauToE[kLogE][jLogE]+
                        FstauToStau[iLogE][kLogE]*FstauToE[kLogE][jLogE];
                }
                temp_stau[3][iLogE][jLogE] += FstauToE[iLogE][jLogE];
            }
        }
        ...
        for(iLogE=0;iLogE<dimension;iLogE++){
            for(jLogE=0;jLogE<=iLogE;jLogE++){
                temp_stau[4][iLogE][jLogE] = 0.0;
                for(kLogE=iLogE;kLogE>=jLogE;kLogE--){
                    temp_stau[4][iLogE][jLogE] += 
                        /** For Glashow Resonance **/
                        FstauToNuE[iLogE][kLogE]*FnuEToMu[kLogE][jLogE]+
                        FstauToNuMu[iLogE][kLogE]*FnuMuToMu[kLogE][jLogE]+
                        FstauToNuTau[iLogE][kLogE]*FnuTauToMu[kLogE][jLogE]+
                        FstauToMu[iLogE][kLogE]*FmuToMu[kLogE][jLogE]+
                        FstauToTau[iLogE][kLogE]*FtauToMu[kLogE][jLogE]+
                        FstauToStau[iLogE][kLogE]*FstauToMu[kLogE][jLogE];
                }
            }
        }
        ...
        for(iLogE=0;iLogE<dimension;iLogE++){
            for(jLogE=0;jLogE<=iLogE;jLogE++){
                temp_stau[5][iLogE][jLogE] = 0.0;
                for(kLogE=iLogE;kLogE>=jLogE;kLogE--){
                    temp_stau[5][iLogE][jLogE] += 
                        FstauToNuE[iLogE][kLogE]*FnuEToTau[kLogE][jLogE]+
                        FstauToNuMu[iLogE][kLogE]*FnuMuToTau[kLogE][jLogE]+
                        FstauToNuTau[iLogE][kLogE]*FnuTauToTau[kLogE][jLogE]+
                        FstauToMu[iLogE][kLogE]*FmuToTau[kLogE][jLogE]+
                        FstauToTau[iLogE][kLogE]*FtauToTau[kLogE][jLogE]+
                        FstauToStau[iLogE][kLogE]*FstauToTau[kLogE][jLogE];
                }
            }
        }
        ...
        for(iLogE=0;iLogE<dimension;iLogE++){
            for(jLogE=0;jLogE<=iLogE;jLogE++){
                temp_stau[6][iLogE][jLogE] = 0.0;
                for(kLogE=iLogE;kLogE>=jLogE;kLogE--){
                    temp_stau[6][iLogE][jLogE] += 
                        FstauToNuE[iLogE][kLogE]*FnuEToHadron[kLogE][jLogE]+
                        FstauToNuMu[iLogE][kLogE]*FnuMuToHadron[kLogE][jLogE]+
                        FstauToNuTau[iLogE][kLogE]*FnuTauToHadron[kLogE][jLogE]+
                        FstauToMu[iLogE][kLogE]*FmuToHadron[kLogE][jLogE]+
                        FstauToTau[iLogE][kLogE]*FtauToHadron[kLogE][jLogE]+
                        FstauToStau[iLogE][kLogE]*FstauToHadron[kLogE][jLogE];
                }
                temp_stau[6][iLogE][jLogE] += FstauToHadron[iLogE][jLogE];
            }
        }
        ...
        for(iLogE=0;iLogE<dimension;iLogE++){
            for(jLogE=0;jLogE<=iLogE;jLogE++){
                temp_stau[7][iLogE][jLogE] = 0.0;
                for(kLogE=iLogE;kLogE>=jLogE;kLogE--){
                    temp_stau[7][iLogE][jLogE] +=
                        FstauToStau[iLogE][kLogE]*FstauToStau[kLogE][jLogE];
                }
            }
        }
        ...
        for(iLogE=0;iLogE<dimension;iLogE++){
            for(jLogE=0;jLogE<=iLogE;jLogE++){
                ...
                /** Stau related **/
                FstauToNuE[iLogE][jLogE] = temp_stau[0][iLogE][jLogE];
                FstauToNuMu[iLogE][jLogE] = temp_stau[1][iLogE][jLogE];
                FstauToNuTau[iLogE][jLogE] = temp_stau[2][iLogE][jLogE];
                FstauToE[iLogE][jLogE] = temp_stau[3][iLogE][jLogE];
                FstauToMu[iLogE][jLogE] = temp_stau[4][iLogE][jLogE];
                FstauToTau[iLogE][jLogE] = temp_stau[5][iLogE][jLogE];
                FstauToHadron[iLogE][jLogE] = temp_stau[6][iLogE][jLogE];
                FstauToStau[iLogE][jLogE] = temp_stau[7][iLogE][jLogE];
            }
        }
    ...
    public void propagateX( ){
        ...
        for(iLogE=0;iLogE<dimension;iLogE++){
            for(jLogE=0;jLogE<=iLogE;jLogE++){
                ...
                for(kLogE=iLogE;kLogE>=jLogE;kLogE--){
                    temp_stau[0][iLogE][jLogE] += 
                        FstauToNuE[iLogE][kLogE]*nuEToNuE[kLogE][jLogE]+
                        FstauToNuMu[iLogE][kLogE]*nuMuToNuE[kLogE][jLogE]+
                        FstauToNuTau[iLogE][kLogE]*nuTauToNuE[kLogE][jLogE]+
                        FstauToMu[iLogE][kLogE]*muToNuE[kLogE][jLogE]+
                        FstauToTau[iLogE][kLogE]*tauToNuE[kLogE][jLogE]+
                        FstauToStau[iLogE][kLogE]*stauToNuE[kLogE][jLogE];
                }
            }
        }
            ...
            for(jLogE=0;jLogE<=iLogE;jLogE++){
                ...
                for(kLogE=iLogE;kLogE>=jLogE;kLogE--){
                    temp_stau[1][iLogE][jLogE] += 
                        FstauToNuE[iLogE][kLogE]*nuEToNuMu[kLogE][jLogE]+
                        FstauToNuMu[iLogE][kLogE]*nuMuToNuMu[kLogE][jLogE]+
                        FstauToNuTau[iLogE][kLogE]*nuTauToNuMu[kLogE][jLogE]+
                        FstauToMu[iLogE][kLogE]*muToNuMu[kLogE][jLogE]+
                        FstauToTau[iLogE][kLogE]*tauToNuMu[kLogE][jLogE]+
                        FstauToStau[iLogE][kLogE]*stauToNuMu[kLogE][jLogE];
                }
            }
        }
            ...
            for(jLogE=0;jLogE<=iLogE;jLogE++){
                ...
                for(kLogE=iLogE;kLogE>=jLogE;kLogE--){
                    temp_stau[2][iLogE][jLogE] += 
                        ...
                        FstauToNuE[iLogE][kLogE]*nuEToNuTau[kLogE][jLogE]+
                        FstauToNuTau[iLogE][kLogE]*nuTauToNuTau[kLogE][jLogE]+
                        FstauToNuMu[iLogE][kLogE]*nuMuToNuTau[kLogE][jLogE]+
                        FstauToMu[iLogE][kLogE]*muToNuTau[kLogE][jLogE]+
                        FstauToTau[iLogE][kLogE]*tauToNuTau[kLogE][jLogE]+
                        FstauToStau[iLogE][kLogE]*stauToNuTau[kLogE][jLogE];
                }
            }
        }
        ...
        for(iLogE=0;iLogE<dimension;iLogE++){
            for(jLogE=0;jLogE<=iLogE;jLogE++){
                ...
                for(kLogE=iLogE;kLogE>=jLogE;kLogE--){
                    temp_stau[3][iLogE][jLogE] += 
                        FstauToNuE[iLogE][kLogE]*nuEToE[kLogE][jLogE]+
                        FstauToNuMu[iLogE][kLogE]*nuMuToE[kLogE][jLogE]+
                        FstauToNuTau[iLogE][kLogE]*nuTauToE[kLogE][jLogE]+
                        FstauToMu[iLogE][kLogE]*muToE[kLogE][jLogE]+
                        FstauToTau[iLogE][kLogE]*tauToE[kLogE][jLogE]+
                        FstauToStau[iLogE][kLogE]*stauToE[kLogE][jLogE];
                }
            ...
            for(jLogE=0;jLogE<=iLogE;jLogE++){
                ...
                for(kLogE=iLogE;kLogE>=jLogE;kLogE--){
                    temp_stau[4][iLogE][jLogE] += 
                        ...
                        FstauToNuE[iLogE][kLogE]*nuEToMu[kLogE][jLogE]+
                        FstauToNuMu[iLogE][kLogE]*nuMuToMu[kLogE][jLogE]+
                        FstauToNuTau[iLogE][kLogE]*nuTauToMu[kLogE][jLogE]+
                        FstauToMu[iLogE][kLogE]*muToMu[kLogE][jLogE]+
                        FstauToTau[iLogE][kLogE]*tauToMu[kLogE][jLogE]+
                        FstauToStau[iLogE][kLogE]*stauToMu[kLogE][jLogE];
                }
        ...
        for(iLogE=0;iLogE<dimension;iLogE++){
            for(jLogE=0;jLogE<=iLogE;jLogE++){
                ...
                for(kLogE=iLogE;kLogE>=jLogE;kLogE--){
                    temp_stau[5][iLogE][jLogE] += 
                        FstauToNuE[iLogE][kLogE]*nuEToTau[kLogE][jLogE]+
                        FstauToNuMu[iLogE][kLogE]*nuMuToTau[kLogE][jLogE]+
                        FstauToNuTau[iLogE][kLogE]*nuTauToTau[kLogE][jLogE]+
                        FstauToMu[iLogE][kLogE]*muToTau[kLogE][jLogE]+
                        FstauToTau[iLogE][kLogE]*tauToTau[kLogE][jLogE]+
                        FstauToStau[iLogE][kLogE]*stauToTau[kLogE][jLogE];
        ...
        for(iLogE=0;iLogE<dimension;iLogE++){
            for(jLogE=0;jLogE<=iLogE;jLogE++){
                ...
                for(kLogE=iLogE;kLogE>=jLogE;kLogE--){
                    temp_stau[7][iLogE][jLogE] +=
                        FstauToStau[iLogE][kLogE]*stauToStau[kLogE][jLogE];
                }
            }
        }
    ...
    public void copyTransferMatrix( ){
        ...
        for(iLogE=0;iLogE<dimension;iLogE++){
            ...
            for(jLogE=0;jLogE<=iLogE;jLogE++){
                ...
                stauToNuE[iLogE][jLogE] = FstauToNuE[iLogE][jLogE];
                // To NuMu
                /** For Glashow Resonance **/
                ...
                stauToNuMu[iLogE][jLogE] = FstauToNuMu[iLogE][jLogE];
                // To NuTau
                /** For Glashow Resonance **/
                ...
                stauToNuTau[iLogE][jLogE] = FstauToNuTau[iLogE][jLogE];
                // To E
                ...
                stauToE[iLogE][jLogE] = FstauToE[iLogE][jLogE];
                // To Mu
                /** For Glashow Resonance **/
                ...
                stauToMu[iLogE][jLogE] = FstauToMu[iLogE][jLogE];
                // To Tau
                /** For Glashow Resonance **/
                ...
                stauToTau[iLogE][jLogE] = FstauToTau[iLogE][jLogE];
                // To Hadron
                ...
                stauToHadron[iLogE][jLogE] = FstauToHadron[iLogE][jLogE]; 
                // To Stau (only stau to stau) 
                stauToStau[iLogE][jLogE] = FstauToStau[iLogE][jLogE];
            }
        }
    ...
    public void storePropagateMatrix( ){
        ...
        for(iLogE=0;iLogE<dimension;iLogE++){
            for(jLogE=0;jLogE<=iLogE;jLogE++){
                ...
                for(kLogE=iLogE;kLogE>=jLogE;kLogE--){
                    temp_stau[0][iLogE][jLogE] +=
                        SstauToNuE[iLogE][kLogE]*FnuEToNuE[kLogE][jLogE]+
                        SstauToNuMu[iLogE][kLogE]*FnuMuToNuE[kLogE][jLogE]+
                        SstauToNuTau[iLogE][kLogE]*FnuTauToNuE[kLogE][jLogE]+
                        SstauToMu[iLogE][kLogE]*FmuToNuE[kLogE][jLogE]+
                        SstauToTau[iLogE][kLogE]*FtauToNuE[kLogE][jLogE]+
                        SstauToStau[iLogE][kLogE]*FstauToNuE[kLogE][jLogE];
                }
            }
        }
        ...
        for(iLogE=0;iLogE<dimension;iLogE++){
            for(jLogE=0;jLogE<=iLogE;jLogE++){
                ...
                for(kLogE=iLogE;kLogE>=jLogE;kLogE--){
                    temp_stau[1][iLogE][jLogE] += 
                        /** For Glshow Resonance **/
                        SstauToNuE[iLogE][kLogE]*FnuEToNuMu[kLogE][jLogE]+
                        SstauToNuMu[iLogE][kLogE]*FnuMuToNuMu[kLogE][jLogE]+
                        SstauToNuTau[iLogE][kLogE]*FnuTauToNuMu[kLogE][jLogE]+
                        SstauToMu[iLogE][kLogE]*FmuToNuMu[kLogE][jLogE]+
                        SstauToTau[iLogE][kLogE]*FtauToNuMu[kLogE][jLogE]+
                        SstauToStau[iLogE][kLogE]*FstauToNuMu[kLogE][jLogE];
                }
            }
        }
        ...
        for(iLogE=0;iLogE<dimension;iLogE++){
            for(jLogE=0;jLogE<=iLogE;jLogE++){
                ...
                for(kLogE=iLogE;kLogE>=jLogE;kLogE--){
                    temp_stau[2][iLogE][jLogE] += 
                        /** For Glashow Resonance **/
                        SstauToNuE[iLogE][kLogE]*FnuEToNuTau[kLogE][jLogE]+
                        SstauToNuMu[iLogE][kLogE]*FnuMuToNuTau[kLogE][jLogE]+
                        SstauToNuTau[iLogE][kLogE]*FnuTauToNuTau[kLogE][jLogE]+
                        SstauToMu[iLogE][kLogE]*FmuToNuTau[kLogE][jLogE]+
                        SstauToTau[iLogE][kLogE]*FtauToNuTau[kLogE][jLogE]+
                        SstauToStau[iLogE][kLogE]*FstauToNuTau[kLogE][jLogE];
                }
            }
        }
        ...
        for(iLogE=0;iLogE<dimension;iLogE++){
            for(jLogE=0;jLogE<=iLogE;jLogE++){
                ...
                for(kLogE=iLogE;kLogE>=jLogE;kLogE--){
                    temp_stau[3][iLogE][jLogE] += 
                        SstauToNuE[iLogE][kLogE]*FnuEToE[kLogE][jLogE]+
                        SstauToNuMu[iLogE][kLogE]*FnuMuToE[kLogE][jLogE]+
                        SstauToNuTau[iLogE][kLogE]*FnuTauToE[kLogE][jLogE]+
                        SstauToMu[iLogE][kLogE]*FmuToE[kLogE][jLogE]+
                        SstauToTau[iLogE][kLogE]*FtauToE[kLogE][jLogE]+
                        SstauToStau[iLogE][kLogE]*FstauToE[kLogE][jLogE];
                }
                temp_stau[3][iLogE][jLogE] += FstauToE[iLogE][jLogE];
            }
        }
        ...
        for(iLogE=0;iLogE<dimension;iLogE++){
            for(jLogE=0;jLogE<=iLogE;jLogE++){
                ...
                for(kLogE=iLogE;kLogE>=jLogE;kLogE--){
                    temp_stau[4][iLogE][jLogE] += 
                        /** For Glahow Resonance **/
                        SstauToNuE[iLogE][kLogE]*FnuEToMu[kLogE][jLogE]+
                        SstauToNuMu[iLogE][kLogE]*FnuMuToMu[kLogE][jLogE]+
                        SstauToNuTau[iLogE][kLogE]*FnuTauToMu[kLogE][jLogE]+
                        SstauToMu[iLogE][kLogE]*FmuToMu[kLogE][jLogE]+
                        SstauToTau[iLogE][kLogE]*FtauToMu[kLogE][jLogE]+
                        SstauToStau[iLogE][kLogE]*FstauToMu[kLogE][jLogE];
                }
            }
        }
        ...
        for(iLogE=0;iLogE<dimension;iLogE++){
            for(jLogE=0;jLogE<=iLogE;jLogE++){
                ...
                for(kLogE=iLogE;kLogE>=jLogE;kLogE--){
                    temp_stau[5][iLogE][jLogE] += 
                        /** For Glahow Resonance **/
                        SstauToNuE[iLogE][kLogE]*FnuEToTau[kLogE][jLogE]+
                        SstauToNuMu[iLogE][kLogE]*FnuMuToTau[kLogE][jLogE]+
                        SstauToNuTau[iLogE][kLogE]*FnuTauToTau[kLogE][jLogE]+
                        SstauToMu[iLogE][kLogE]*FmuToTau[kLogE][jLogE]+
                        SstauToTau[iLogE][kLogE]*FtauToTau[kLogE][jLogE]+
                        SstauToStau[iLogE][kLogE]*FstauToTau[kLogE][jLogE];
                }
            }
        }
        ...
        for(iLogE=0;iLogE<dimension;iLogE++){
            for(jLogE=0;jLogE<=iLogE;jLogE++){
                ...
                for(kLogE=iLogE;kLogE>=jLogE;kLogE--){
                    temp_stau[6][iLogE][jLogE] += 
                        SstauToNuE[iLogE][kLogE]*FnuEToHadron[kLogE][jLogE]+
                        SstauToNuMu[iLogE][kLogE]*FnuMuToHadron[kLogE][jLogE]+
                        SstauToNuTau[iLogE][kLogE]*FnuTauToHadron[kLogE][jLogE]+
                        SstauToMu[iLogE][kLogE]*FmuToHadron[kLogE][jLogE]+
                        SstauToTau[iLogE][kLogE]*FtauToHadron[kLogE][jLogE]+
                        SstauToStau[iLogE][kLogE]*FstauToHadron[kLogE][jLogE]; // only stau can be stau 
                }
                temp_stau[6][iLogE][jLogE] += FstauToHadron[iLogE][jLogE];
            }
        }
        ...
        for(iLogE=0;iLogE<dimension;iLogE++){
            for(jLogE=0;jLogE<=iLogE;jLogE++){
                ...
                for(kLogE=iLogE;kLogE>=jLogE;kLogE--){
                    temp_stau[7][iLogE][jLogE] +=
                        SstauToStau[iLogE][kLogE]*FstauToStau[kLogE][jLogE];
                }
            }
        }
        ...
        for(iLogE=0;iLogE<dimension;iLogE++){
            for(jLogE=0;jLogE<=iLogE;jLogE++){
                ...
                SstauToNuE[iLogE][jLogE] = temp_stau[0][iLogE][jLogE];
                // To NuMu
                /** For Glashow Resonance **/
                ...
                SstauToNuMu[iLogE][jLogE] = temp_stau[1][iLogE][jLogE];
                // To NuTau
                /** For Glashow Resonance **/
                ...
                SstauToNuTau[iLogE][jLogE] = temp_stau[2][iLogE][jLogE];
                // To E
                ...
                SstauToE[iLogE][jLogE] = temp_stau[3][iLogE][jLogE];
                // To Mu
                /** For Glashow Resonance **/
                ...
                SstauToMu[iLogE][jLogE] = temp_stau[4][iLogE][jLogE];
                // To Tau
                /** For Glashow Resonance **/
                ...
                SstauToTau[iLogE][jLogE] = temp_stau[5][iLogE][jLogE];
                // To Hadron
                ...
                SstauToHadron[iLogE][jLogE] = temp_stau[6][iLogE][jLogE]; 
                // To Stau (only stau to stau)
                SstauToStau[iLogE][jLogE] = temp_stau[7][iLogE][jLogE];
            }
        }
    ...
    public void copyTransferMatrixFromStore( ){
        ...
        for(iLogE=0;iLogE<dimension;iLogE++){
            ...
            for(jLogE=0;jLogE<=iLogE;jLogE++){
                ...
                FstauToNuE[iLogE][jLogE] = SstauToNuE[iLogE][jLogE];
                // To NuMu
                /** For Glashow Resonance **/
                ...
                FstauToNuMu[iLogE][jLogE] = SstauToNuMu[iLogE][jLogE];
                // To NuTau
                /** For Glashow Resonance **/
                /** Modified ! **/
                ...
                FstauToNuTau[iLogE][jLogE] = SstauToNuTau[iLogE][jLogE];
                // To E
                ...
                FstauToE[iLogE][jLogE] = SstauToE[iLogE][jLogE];
                // To Mu
                /** For Glashow Resonance **/
                ...
                FstauToMu[iLogE][jLogE] = SstauToMu[iLogE][jLogE];
                // To Tau
                /** For Glashow Resonance **/
                ...
                FstauToTau[iLogE][jLogE] = SstauToTau[iLogE][jLogE];
                // To Hadron
                ...
                FstauToHadron[iLogE][jLogE] = SstauToHadron[iLogE][jLogE];
                // To Stau (only stau to stau)
                FstauToStau[iLogE][jLogE] = SstauToStau[iLogE][jLogE];
            }
        }
    ...
    /** Stau related **/
    public double getFstauToNuE(int iLogE, int jLogE){
        if((0<=iLogE && iLogE<dimension)&&(0<=jLogE && jLogE<dimension)){
            return FstauToNuE[iLogE][jLogE];
        }
        else{
            return 0.0;
        }
    ...
    public double getFstauToNuMu(int iLogE, int jLogE){
        if((0<=iLogE && iLogE<dimension)&&(0<=jLogE && jLogE<dimension)){
            return FstauToNuMu[iLogE][jLogE];
        }
        else{
            return 0.0;
        }
    ...
    public double getFstauToNuTau(int iLogE, int jLogE){
        if((0<=iLogE && iLogE<dimension)&&(0<=jLogE && jLogE<dimension)){
            return FstauToNuTau[iLogE][jLogE];
        }
        else{
            return 0.0;
        }
    ...
    public double getFstauToE(int iLogE, int jLogE){
        if((0<=iLogE && iLogE<dimension)&&(0<=jLogE && jLogE<dimension)){
            return FstauToE[iLogE][jLogE];
        }
        else{
            return 0.0;
        }
    ...
    public double getFstauToMu(int iLogE, int jLogE){
        if((0<=iLogE && iLogE<dimension)&&(0<=jLogE && jLogE<dimension)){
            return FstauToMu[iLogE][jLogE];
        }
        else{
            return 0.0;
        }
    ...
    public double getFstauToTau(int iLogE, int jLogE){
        if((0<=iLogE && iLogE<dimension)&&(0<=jLogE && jLogE<dimension)){
            return FstauToTau[iLogE][jLogE];
        }
        else{
            return 0.0;
        }
    ...
    public double getFstauToHadron(int iLogE, int jLogE){
        if((0<=iLogE && iLogE<dimension)&&(0<=jLogE && jLogE<dimension)){
            return FstauToHadron[iLogE][jLogE];
        }
        else{
            return 0.0;
        }
    ...
    public double getFstauToStau(int iLogE, int jLogE){
        if((0<=iLogE && iLogE<dimension)&&(0<=jLogE && jLogE<dimension)){
            return FstauToStau[iLogE][jLogE];
        }
        else{
            return 0.0;
        }
    ...
    public void setStauMass(int setmass){
        this.staumass = setmass; 
        this.stauToEPairCMtxObjectFile = "stauToePairCreationMtx" + String.valueOf(staumass) + "GeV";
        this.stauToMuPairCMtxObjectFile = "stauTomuPairCreationMtx" + String.valueOf(staumass) + "GeV";
        this.stauToTauPairCMtxObjectFile = "stauTotauPairCreationMtx" + String.valueOf(staumass) + "GeV";
        return;
